...
bg_specie <- dismo::randomPoints(mask = var, n = nrow(pr_specie)) %>% # mask - limite de amostragem e n - pontos de pseudo-ausencia
  tibble::as_tibble() %>%
...
names(sdm) <- c("Bioclim", "Bioclim_binario", "GLM", "GLM_binario", "Maxent", "Maxent_binario")
sdm
